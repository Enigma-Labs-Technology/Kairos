...
async function assertWebhookSubscriberUrlAllowed(subscriberUrl: string) {
  const validation = await validatePublicUrlForSSRF(subscriberUrl);
  if (!validation.isValid) {
    throw ErrorWithCode.Factory.BadRequest(`Webhook URL is not allowed: ${validation.error}`);
  }
}
...
export async function addSubscription({
  ...
  try {
    await assertWebhookSubscriberUrlAllowed(subscriberUrl);
